...
void APP_Tasks(void) {
    ...
    switch (appData.state) {
        ...
        case APP_STATE_MOUSE_EMULATE:
            ...
            f_ay=(float)ay/32768.0*MULTIPLIER;
            f_ax=(float)ax/32768.0*MULTIPLIER;
            
            int8_t f_ay_s = -f_ay*SCALING_FACTOR;
            int8_t f_ax_s = f_ax*SCALING_FACTOR;
            
            sprintf(output,"ax: %0.2f, ay: %0.2f    ",f_ax,f_ay);
            drawString(10,8,output,WHITE,BLUE);
            sprintf(output,"az: %0.2f   ",f_az);
            drawString(10,16,output,WHITE,BLUE);
            // every 50th loop, or 20 times per second
            if (movement_length > 10) {
                appData.mouseButton[0] = MOUSE_BUTTON_STATE_RELEASED;
                appData.mouseButton[1] = MOUSE_BUTTON_STATE_RELEASED;
                appData.xCoordinate = (int8_t) f_ay_s;
                appData.yCoordinate = (int8_t) f_ax_s;
                vector++;
                movement_length = 0;
            }
